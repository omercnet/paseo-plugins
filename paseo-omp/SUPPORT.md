...
- OMP 18.2 does not positively correlate ordinary `prompt` requests with their eventual `agent_end`: the immediate response omits `agentInvoked: true`, `prompt_result` reports only `false`, and live user messages may omit their persisted `entryId`. For ownership-sensitive later turns, the plugin uses a bounded `get_branch_messages` watermark and accepts terminal completion only after a new exact-text branch entry appears. If that snapshot is unavailable, oversized, or ambiguous, the plugin fails closed. A request-correlated positive prompt result or terminal event remains the upstream protocol fix that would remove this compatibility boundary.

...
- Paseo: `^0.9.2` on both the daemon and every app loading the client entry. The 0.9.2 floor includes the plugin-provider request, reload-cleanup, and daemon-shutdown fixes required for reliable OMP provider operation.
...
- Paseo 0.9.2 or newer is required for reliable plugin-provider lifecycle handling, platform-owned plugin URL opening, and image-card transformation before Overview grouping.

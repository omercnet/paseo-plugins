...
async function validateAgentBrowser(path) {
	...
	if (version !== expectedVersion) {
		throw new Error(
			`Expected agent-browser ${expectedVersion}, received ${version ?? "unknown"}`,
		);
	}
}
...
async function installChromium() {
	...
	if (!releaseDir)
		throw new Error("agent-browser install did not produce a Chromium release");
...
async function stageAgentBrowserEntry() {
	if (hostPlatform === "win32") {
		...
		if (executableArch !== "x64") {
			throw new Error(
				`Unsupported agent-browser Windows architecture: ${hostArch}`,
			);
		}
		await cp(
			join(
				packagedAgentBrowser,
				"bin",
				`agent-browser-win32-${executableArch}.exe`,
			),
			runtimeEntry,
			{ force: true },
		);
...
if (binaryOverride)
	await validateAgentBrowser(
		await requireExecutable(
			binaryOverride,
			"PASEO_SHARED_BROWSER_AGENT_BROWSER_BINARY",
		),
	);
await validateAgentBrowser(
	await requireExecutable(runtimeEntry, "Packaged agent-browser entry"),
);
...
if (hostPlatform !== "win32") {
	await validateChromium(
		await requireExecutable(chromiumEntry, "Staged Chromium executable"),
	);
}
await rename(stagingRoot, runtimeRoot);

console.log(
	`Prepared immutable Shared Browser runtime assets in ${runtimeRoot}`,
);

	...
	static connect(
		...
		const WebSocketImpl =
			...
			(globalThis as typeof globalThis & { WebSocket?: WebSocketConstructor })
				.WebSocket;
		if (!WebSocketImpl)
			throw new CdpUnavailableError("This runtime does not provide WebSocket");
		...
		const timer = setTimeout(() => {
			...
			reject(
				new CdpUnavailableError(
					`CDP connection timed out after ${timeoutMs}ms`,
				),
			);
		}, timeoutMs);
		socket.onopen = () => {
	...
	async send<T = unknown>(
			...
			reject(
				mutation
					? new CdpUnknownOutcomeError(
							`${method} timed out; mutation outcome is unknown`,
						)
		...
		try {
			...
			reject(
				mutation
					? new CdpUnknownOutcomeError(
							`${method} send failed; mutation outcome is unknown`,
						)
					: error,
			);
		}
	...
	private rejectPending(message: string): void {
		for (const pending of this.pending.values()) {
			...
			pending.reject(
				pending.mutation
					? new CdpUnknownOutcomeError(
							`${pending.method} interrupted; mutation outcome is unknown`,
						)
...
export async function listPageTargets(
	...
	const result = await connection.send<{ targetInfos: CdpTarget[] }>(
		"Target.getTargets",
	);

...
async function main(): Promise<void> {
	...
	server.registerTool(
		...
		{
			description:
				"Read the current shared browser state for this agent's workspace.",
	...
	server.registerTool(
		...
		{
			description:
				"Navigate the shared browser using the agent's current observed state.",
			inputSchema: z.object({
	...
	server.registerTool(
		...
		async ({ event }) =>
			textResult(
				await client.request("input", { event: event as unknown as JsonValue }),
			),
	);
	...
	server.registerTool(
		...
		{
			description:
				"Resize the shared browser viewport using the agent's current observed state.",
			inputSchema: z.object({

...
export function isBrowserStateCurrent(
...
export function didBrowserRuntimeRestart(
	previous: BrowserState,
	next: BrowserState,
): boolean {
	return Boolean(
...
export function mapDisplayedPoint(
	...
	return {
		x: Math.min(
			viewport.width - 1,
			Math.max(0, (point.x / point.width) * viewport.width),
		),
		y: Math.min(
			viewport.height - 1,
			Math.max(0, (point.y / point.height) * viewport.height),
		),
	};
}

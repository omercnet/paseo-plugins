...
export default function contribute(server: PluginServerContext) {
	...
	server.before("agent.session_open", async ({ request }) => {
		...
		if (!ticket || request.purpose !== "interactive" || !request.workspaceId)
			return request;
	...
	server.on("agent.archived", ({ agent }) =>
		revokeAgentBrowserAccess(agent.id),
	);

	server.on("workspace.archived", ({ workspace }) =>
		handleWorkspaceArchived(workspace.id),
	);

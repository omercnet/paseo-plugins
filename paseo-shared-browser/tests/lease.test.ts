...
class FakeSupervisorClient {
	...
	async requestWorkspace(
		...
		switch (operation) {
			...
			case "mouse.down":
				if (workspace.navigateOnMouseDown)
					workspace.url = "https://navigated.example/";
				return null;
			case "mouse.up":
...
function createManager(
	...
	const manager = new SessionManager({
		...
		validateWorkspace: async (workspaceId) =>
...
describe("SessionManager control leases", () => {
	it("shares one runtime, excludes a second controller, and supports takeover", async () => {
		...
		expect(
			(await manager.capture(first.viewerToken, "medium", null)).frame,
		).toMatchObject({
			runtimeId: "runtime-workspace-one",
			captureEpoch: 1,
		});
		...
		await expect(
			manager.acquireControl(second.viewerToken, false),
		).rejects.toThrow("held by another viewer");
		const secondControl = await manager.acquireControl(
			second.viewerToken,
			true,
		);
		...
		await expect(
			manager.capture(first.viewerToken, "medium", null),
	...
	it("expires abandoned control and viewer leases deterministically", async () => {
		...
		now += 1_001;
		expect(
			(await manager.capture(viewer.viewerToken, "medium", null)).state
				.controller,
		).toBe("none");
		now += 10_001;
		await expect(
			manager.capture(viewer.viewerToken, "medium", null),
		).rejects.toThrow("invalid or expired");
	});
	...
	it("accepts a shared recent frame and rejects it after viewport invalidation", async () => {
		...
		const frame = (await manager.capture(second.viewerToken, "medium", null))
			.frame!;
	...
	it("rejects stale navigation, viewport, runtime, and bridge generations", async () => {
		...
		await expect(
			navigate({ ...current, runtimeId: "different-runtime" }),
		).rejects.toThrow("runtime is stale");
		await expect(
			navigate({ ...current, bridgeEpoch: current.bridgeEpoch! + 1 }),
		).rejects.toThrow("bridge state is stale");
	});
	...
	it("applies device state through the supervisor runtime", async () => {
		...
		const emulate = client.operations
			.filter(({ operation }) => operation === "emulate")
			.at(-1);
	...
	it("archives and tears down the workspace runtime", async () => {
		...
		await expect(
			manager.attach("workspace-archive", "Late client"),
	...
	it("cannot create a runtime after disconnect begins", async () => {
		...
		const started = new Promise<void>(
			(resolve) => (validationStarted = resolve),
		);
		const client = new FakeSupervisorClient();
		const manager = new SessionManager({
			client: client as unknown as SupervisorClient,
	...
	it("reports only workspaces with attached viewers and bounds live sessions", async () => {
		...
		expect(await manager.listOpenWorkspaceIds()).toEqual([]);
		await expect(
			manager.attach("workspace-two", "Second client"),
		).rejects.toThrow("session limit (1) reached");
	});

	it("lets archive win while workspace startup is in flight", async () => {
		let releaseEnsure!: () => void;
		let markEnsureStarted!: () => void;
		const ensureStarted = new Promise<void>(
			(resolve) => (markEnsureStarted = resolve),
		);
		const { manager, client } = createManager();
		client.ensureStarted = markEnsureStarted;
		client.ensureGate = new Promise<void>(
		...
		await expect(
			manager.attach("workspace-racing", "Late client"),
		).rejects.toThrow("archived");
	});
	...
	it("archives an ensured runtime when session initialization fails", async () => {
		...
		await expect(manager.attach("workspace-failed", "Client")).rejects.toThrow(
			"Failed identity",
		);
